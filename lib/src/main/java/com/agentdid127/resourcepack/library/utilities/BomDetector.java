 ...
 * 
 * <P>
 * Dev tool only. If you use this tool to remove BOMs, please ensure
 * you have made a backup.
 * 
 * <P>
 * This class assumes the UTF-8 encoding for the BOM, but
...
public final class BomDetector {
    ...
    public BomDetector(String rootDirectory, String... fileExtensions) {
        ...
        extensions = Arrays.asList(fileExtensions);
    ...
    public List<String> findBOMs() throws IOException {
        ...
        for (File textFile : this.findTextFilesBeneath(this.rootDir))
            if (this.startsWithBOM(textFile))
    ...
    public List<String> removeBOMs() throws IOException {
        List<String> result = new ArrayList<String>();
        for (String bomFile : this.findBOMs()) {
          this.stripBomFrom(bomFile);
            result.add(bomFile);
        }
        return result;
    }
    ...
    private List<File> findTextFilesBeneath(File startingDir) throws IOException {
        ...
        File[] filesDirs = filesAndDirs;
        for (File file : filesDirs) {
            if (this.isTextFile(file))
                result.add(file);
            if (file.isDirectory()) {
                // recursive call!!
                List<File> deeperList = this.findTextFilesBeneath(file);
    ...
    private boolean isTextFile(File file) throws IOException {
        ...
        if (finalDot > -1) {
            ...
            result = this.extensions.contains(extension);
        }
        return result;
    }
    ...
    private boolean startsWithBOM(File textFile) throws IOException {
        boolean result = false;
        if (textFile.length() < this.BYTE_ORDER_MARK.length)
            return false;
        // open as bytes here, not characters
        int[] firstFewBytes = new int[this.BYTE_ORDER_MARK.length];
        InputStream input = null;
        try {
            input = new FileInputStream(textFile);
            for (int index = 0; index < this.BYTE_ORDER_MARK.length; ++index)
                firstFewBytes[index] = input.read(); // read a single byte
            result = Arrays.equals(firstFewBytes, this.BYTE_ORDER_MARK);
        } finally {
            input.close();
        }
        return result;
    ...
    private void stripBomFrom(String textFile) throws IOException {
        ...
        long truncatedSize = initialSize - this.BYTE_ORDER_MARK.length;
        byte[] memory = new byte[(int) (truncatedSize)];
        InputStream input = null;
        try {
            input = new BufferedInputStream(new FileInputStream(bomFile));
            input.skip(this.BYTE_ORDER_MARK.length);
          ...
          this.overwriteWithoutBOM(memory, bomFile);
        } finally {
            input.close();
        }
        ...
        if (changeInSize != this.BYTE_ORDER_MARK.length) {
            throw new RuntimeException(
                    "Change in file size: " + changeInSize + " Expected change: " + this.BYTE_ORDER_MARK.length);
        }
    }
